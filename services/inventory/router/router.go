...
import (
	...
	"microservice-challenge/services/inventory/docs"
	"microservice-challenge/services/inventory/httphandler"
	"microservice-challenge/services/inventory/router/internal"
	"net/http"
	"time"
...
func NewRouter(handler *httphandler.Handler, logger log.Logger, jwtSecret string, db *sql.DB) http.Handler {
	...
	docs.SwaggerInfo.Host = "localhost:8003"
	...
	healthCheck := health.New("inventory-service", "1.0.0")
	if db != nil {
	...
	r.Route("/", func(r chi.Router) {
		...
		internal.InitInventoryRoutes(r, handler, authMiddleware)
	})
